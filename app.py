...
# --- Lazy import for socketio (avoid cost if not used) ---
try:
...
print("✅ Model and MediaPipe initialized.")
...
@app.route("/")
def home():
    return jsonify({"message": "🖐 ASL backend ready."})
...
def predict():
    ...
    print(f"🖐 {smooth} ({conf*100:.1f}%)")
    return jsonify({"prediction": smooth, "confidence": conf})


# --- Optional Socket.IO live streaming ---
if socketio:
